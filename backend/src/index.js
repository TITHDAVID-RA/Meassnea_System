...
export default {
  async fetch(request, env, ctx) {
    ...
    // --- CASE AGNOSTIC SANITIZATION HELPERS ---
    // Safely reads a key whether it's camelCase or snake_case from the payload
    const getVal = (obj, camelKey, snakeKey, defaultVal = '') => {
    ...
    const getNum = (obj, camelKey, snakeKey, defaultVal = 0) => {
    ...
    // --- REUSABLE MATERIAL LEDGER FUNCTIONS ---
    const getMaterialUnitCost = async (materialName, size) => {
    ...
    const deductPlasticBag = async (size, qty, orderNumber) => {
    ...
    const returnPlasticBag = async (size, qty, orderNumber) => {
      const material = await env.meassnea_db.prepare(
        "SELECT * FROM materials WHERE name = 'plastic_bag' AND size = ? LIMIT 1"
      ).bind(size).first();
      ...
      await env.meassnea_db.prepare(`
        INSERT INTO material_transactions (id, material_id, material_name, size, quantity, unit_price, total_price, type, notes, transaction_date)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      `).bind(txId, material.id, 'plastic_bag', size, qty, material.unit_price, qty * material.unit_price, 'return', `Order cancellation ${orderNumber}`, new Date().toISOString()).run();

      return { material_id: material.id, previous_quantity: material.quantity, new_quantity: newQty };
    };

    const calculatePlasticBagCost = async (plasticBags) => {
      let totalCost = 0;
    ...
    const deductProductMaterials = async (productId, quantity, orderNumber) => {
      ...
      if (product.name === 'ទាបបារាំង') {
        ...
        if (material) {
          ...
          await env.meassnea_db.prepare(`
            INSERT INTO material_transactions (id, material_id, material_name, size, quantity, unit_price, total_price, type, notes, transaction_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(txId, material.id, 'ទាបបារាំង', 'N/A', -quantity, material.unit_price, -(quantity * material.unit_price), 'deduction', `Order ${orderNumber} - ${product.name}`, new Date().toISOString()).run();
          deductions.push({ material_id: material.id, name: 'ទាបបារាំង', new_quantity: newQty });
        }
        return deductions;
      }

      // Sized Standard Production Materials
      ...
      for (const matName of materialsToDeduct) {
        ...
        if (material) {
          ...
          await env.meassnea_db.prepare(`
            INSERT INTO material_transactions (id, material_id, material_name, size, quantity, unit_price, total_price, type, notes, transaction_date)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
          `).bind(txId, material.id, matName, material.size || 'N/A', -quantity, material.unit_price, -(quantity * material.unit_price), 'deduction', `Order ${orderNumber} - ${product.name}`, new Date().toISOString()).run();
    ...
    try {
      ...
      if (path === "/api/material-transactions") {
        ...
        if (method === "POST") {
          ...
          const transaction_date = getVal(body, 'date', 'transaction_date', new Date().toISOString());
      ...
      if (path === "/api/orders") {
        ...
        if (method === "POST") {
          ...
          // Parse plasticBags - can be array or JSON string from store
      ...
      if (path === "/api/incomes") {
        ...
        if (method === "POST") {
          ...
          const income_date = getVal(body, 'date', 'income_date', new Date().toISOString());
      ...
      if (path === "/api/expenses") {
        ...
        if (method === "POST") {
          ...
          const reference = getVal(body, 'reference', 'reference', '');
          const expense_date = getVal(body, 'date', 'expense_date', new Date().toISOString());
